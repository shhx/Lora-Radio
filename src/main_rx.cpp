...
void loop() {
    ...
    delay(200);
    // sx1281_set_mode(SX1280_MODE_RX, 0);
    while (digitalRead(RADIO_DIO1) == LOW) {
        delay(400);
        uint16_t irq_mask = sx1281_get_irq_status();
        Serial.print("IRQ: 0x");
        Serial.print(irq_mask, HEX);
        uint8_t status = sx1281_get_status();
        Serial.print(" Status: 0x");
        Serial.print(RADIO_STATE(status), HEX);
        Serial.print(" 0x");
        Serial.println(CMD_STATUS(status), HEX);
        uint8_t payload_len = sx1281_get_rx_buf_addr();
        Serial.print("Payload Len: ");
        Serial.println(payload_len);
    }
    Serial.println();
    SX1280_PacketStatusLoRa_t status;
    sx1281_get_packet_status_lora(&status);
    uint16_t irq_mask = sx1281_get_irq_status();
    Serial.print("IRQ: 0x");
    Serial.println(irq_mask, HEX);
    sx1281_clear_irq_status(irq_mask);
    uint8_t payload_len = sx1281_get_rx_buf_addr();
